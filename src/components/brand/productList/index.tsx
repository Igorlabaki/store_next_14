...
import { brandProductsListAction } from "@/serverActions/brand/productList";
...
export async function BrandProductListComponent({brandId, query}:BrandProductListProps) {
    ...
    <div className="w-full px-[1rem] py-[3.5rem] flex flex-col gap-y-5">
        <SearchComponent searchUrl={`/brand/${brandId}?search=`}/>
        {
            brandProductsList.map((item:Product) => {
                return(
                    <BrandProductItemListComponent product={item} key={item.id}/>
                )
            })
        }
    </div>
